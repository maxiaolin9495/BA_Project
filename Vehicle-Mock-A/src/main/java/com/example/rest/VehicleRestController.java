...
public class VehicleRestController {
    ...
    @RequestMapping(method = RequestMethod.GET, path = "manipulation/getToken")
    @ApiResponses({
    ...
    @RequestMapping(method = RequestMethod.GET, path = "manipulation/sendMessage")
    @ApiResponses({
            @ApiResponse(code = 200,
                    message = "Verified vehicle",
                    response = TokenResponse.class),
    ...
    public ResponseEntity<DataResponse> sendMessage() throws Exception {
        log.info("start send message");
        if(validToken == null){
            validToken = requestTokenService.requestToken();
        }
        DataResponse dataResponse = dataService.sendData(validToken);
